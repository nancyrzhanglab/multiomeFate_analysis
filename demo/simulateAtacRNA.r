...
library(umap) # for its umap function.
...
# plot it!
plot(c(activeInterval[,1], activeInterval[,2]),rep(1:nrow(activeInterval),2), xlab="Time", ylab="Phase", cex=2, pch=18)
for(i in 1:nrow(activeInterval)) segments(activeInterval[i,1], i, activeInterval[i,2],i)
...
# plot it!
plot(genePhase,xlab="Gene Index", ylab="Active Phase", col=geneBranch+1, 
     main="Active phase of each gene, colored by branch")
...
par(mfrow=c(nBranches+1,1))
par(mar=c(5.1, 4.1, 4.1, 2.1))
image.plot(1:timesteps, 1:(sum(geneBranch==0)*nRegionsPerGene),t(flatten(chromatinOpen[[2]][which(geneBranch==0),,])),
           xlab="Time", ylab="Peak", main="Peaks of shared genes in branch 2 cells")
for(b in 1:nBranches){
  image.plot(1:timesteps, 1:(sum(geneBranch==b)*nRegionsPerGene),t(flatten(chromatinOpen[[2]][which(geneBranch==b),,])),
             zlim=c(0,1),xlab="Time", ylab="Peak", main=paste("Peaks of branch",b,"genes in branch 2 cells"))
}
...
png("simdata.png", height=800, width=1200)
