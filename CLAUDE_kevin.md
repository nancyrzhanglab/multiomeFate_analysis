...
- Spread-size coupling (Jensen's inequality on the log-normal mean, `E[Y_l] ∝ exp(β² σ_l²/2)`): with isotropic clone-varying spread, diffuse clones expand more (Spearman(AED_l, Y_l) up to ~0.6 at the top AED level). Calibrated out of the Gini, but the coupling stays in the data unless the spread variation is moved to the non-causal coordinates (open question).
