...
- Open: the 6 questions in §9 (`κ = 1.5` fixed vs growing along the axis; isotropic vs non-causal-only clone-varying spread, i.e. keep the Jensen coupling or not; provisional fixed values 0.5 / 0.4; the two level lists; whether the `τ_δ` row runs in the trailblazing round; confirm the rank-biserial formula).
