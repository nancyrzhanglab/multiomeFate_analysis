...
all_data <- multiomeFate:::data_loader(which_files = c("peakvi", "fasttopics"))
...
rna_features <- all_data[[paste0("fasttopic.", treatment)]]@cell.embeddings
rna_features <- scale(rna_features)
...
fit_res <- multiomeFate:::lineage_cv(
  ...
  tab_mat = tab_mat,
  num_folds = 20,
  savefile_tmp = paste0(out_folder, "Writeup10a_", treatment, "-from-", day_early, "_fatepotential_tmp.RData"),
  verbose = 4
)

save(date_of_run, session_info,
     fit_res,
     file = paste0(out_folder, "Writeup10a_", treatment, "-from-", day_early, "_fatepotential.RData"))
...
save(date_of_run, session_info,
     ...
     tab_mat,
     treatment,
     file = paste0(out_folder, "Writeup10a_", treatment, "-from-", day_early, "to-", day_later, "_fatepotential.RData"))
