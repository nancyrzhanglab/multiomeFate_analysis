...
plot1 <- ggtern::ggtern(data = df,
  ...
  ggplot2::labs(x = "Monocyte", 
                ...
                z = "Undifferentiated",
                title = paste0("Percent fate for day ", day_early))
ggtern::ggsave(filename = paste0("~/project/Multiome_fate/out/figures/Writeup9/Writeup9_percent-fate_", day_early, "_cell-pairs_by-dominate-fate.png"),
               plot1, 
               device = "png", 
               width = 10, 
               height = 5, 
...
# what is connection between current state celltype and its future fate?

print(paste0("Result from ", day_early, " to ", day_later))
table(df$celltype, df$dominant_fate)


# let's double-check this...

idx <- which(seurat_object$time_celltype == "Neutrophil-2")
length(idx)
k <- 2
colnames(seurat_object)[idx[k]]
seurat_object$time_celltype[idx[k]]
lineage <- seurat_object$assigned_lineage[idx[k]]
table(seurat_object$time_celltype[which(seurat_object$assigned_lineage == lineage)])
