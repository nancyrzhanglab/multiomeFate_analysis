...
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_frontrunner-", min_cell_count, ".png"),
...
png(paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_frontrunner-", min_cell_count, "_synchrony.png"),
...
png(paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_synchrony.png"),
    height = 2000, width = 2000, units = "px", res = 500)
plot(x = dabtram_only@reductions[["umap"]]@cell.embeddings[,1],
     y = dabtram_only@reductions[["umap"]]@cell.embeddings[,2],
     col = color_vec, pch = 16,
...
cells.2 <- which(!dabtram_only$assigned_lineage %in% sel.lineages 
                 & dabtram_only$assigned_posterior > 0.5
                 & dabtram_only$dataset == earlierday_cond_str)

png(paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_synchrony-histogram.png"),
...
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_umap_common.png"),
...
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_umap_distinct1.png"),
...
gene_vec <- gene_vec[which(gene_vec %in% colnames(multiSVD_obj$common_mat_1))]
length(gene_vec)
...
atac_pred <- tcrossprod(tiltedCCA:::.mult_mat_vec(svd_2$u %*% rotation_mat, svd_1$d), svd_1$v)
n <- nrow(rna_common)
gene_idx <- which(colnames(rna_common) %in% gene_vec)
alignment_vec <- sapply(1:n, function(i){
  if(i %% floor(n/10) == 0) cat('*')
  
  df <- data.frame(rna = rna_common[i,gene_idx],
                   atac = atac_pred[i,gene_idx])
  lm_res <- stats::lm(rna ~ atac, data = df)
  summary(lm_res)$r.squared
})
...
png(paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_frontrunner-", min_cell_count, "_synchrony2.png"),
    height = 2000, width = 2000, units = "px", res = 500)
plot(x = dabtram_only@reductions[["umap"]]@cell.embeddings[,1],
     y = dabtram_only@reductions[["umap"]]@cell.embeddings[,2],
     col = color_vec, pch = 16,
...
png(paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_synchrony2.png"),
    height = 2000, width = 2000, units = "px", res = 500)
plot(x = dabtram_only@reductions[["umap"]]@cell.embeddings[,1],
     y = dabtram_only@reductions[["umap"]]@cell.embeddings[,2],
     col = color_vec, pch = 16,
...
png(paste0("../../../../out/figures/Writeup6c/Writeup6c_tcca-RNA-ATAC_DABTRAM_synchrony2-histogram.png"),
    height = 1500, width = 3000, units = "px", res = 500)
par(mfrow = c(1,2), mar = c(4,4,4,0.5))
