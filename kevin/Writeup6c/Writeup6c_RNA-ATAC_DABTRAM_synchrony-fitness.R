...
target_genes <- gene_vec[which(gene_vec %in% colnames(rna_common))]
...
for(kk in 1:3){
  png(paste0("../../../../out/figures/Writeup6c/Writeup6c_RNA-ATAC_DABTRAM_synchrony-fitness_", kk, ".png"),
      height = 1500, width = 3000, units = "px", res = 300)
  n <- nrow(pred_diff_mat)
  par(mar = c(4,4,4,0.5), mfrow = c(3,5))
  
  for(i in ((kk-1)*15+1):(kk*15)){
...
# plot some of the important genes

######################

# compute the differentiability for each gene, between cells that will eventually thrive vs definitely not

# plot the genes, the r^2 regression score against differentiability
