...
all(rownames(rna_mat) %in% gene_vec)
...
result_list_tmp <- result_list[colnames(rna_mat)]

for(i in 1:length(result_list_tmp)){
  gene <- names(result_list_tmp)[i]
  mat <- result_list_tmp[[i]]
  ...
  rownames(mat4) <- rownames(result_list_tmp[[i]])
  colnames(mat4) <- colnames(result_list_tmp[[i]])
  
  result_list_tmp[[i]] <- mat4
}

# now trim to only the day10s
for(i in 1:length(result_list_tmp)){
  result_list_tmp[[i]] <- result_list_tmp[[i]][rownames(rna_mat),]
}
...
y <- form_onehot_classification_mat(tier_vec)
gene_vec <- names(result_list_tmp)
...
for(i in 1:length(gene_vec)){
  ...
  tmp <- cbind(rna_mat[,gene], result_list_tmp[[gene]])
  tmp <- scale(tmp)
  spca_res_list[[gene]] <- supervised_pca(x = tmp, y = y)
}
