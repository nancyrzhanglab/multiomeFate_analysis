...
treatment <- "COCL2"
gene_vec <- sort(c("EGR3", "FOSL1", "CD151", "JUNB", "KLF4", "LOXL2", "ANXA2", 
                   "WNT5A", "VGF", "SERPINE1", "PDGFRB", "PRNP", "NDRG1", "MYL6"))
...
for(i in 1:ceiling(length(gene_vec)/4)){
  print(paste0("Batch number ", i, " out of ", ceiling(length(gene_vec)/4)))
  gene_vec_small <- gene_vec[((i-1)*4+1):min(i*4, length(gene_vec))]
  ...
  png(paste0("../../../../out/figures/Writeup6d/Writeup6d_", treatment, "_coverage-pileup2_", i, ".png"),
