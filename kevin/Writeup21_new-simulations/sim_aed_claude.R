...
result_list <- run_sweep(
  axis = "aed",
  level_vec = level_vec,
  fixed_val = fixed_gini,
  num_replicates = num_replicates,
  out_dir = out_dir,
  python_path = python_path,
  script_path = file.path(script_dir, "run_cospar.py"),
  rds_file = file.path(out_dir, paste0("sim_aed_claude", suffix, ".rds")),
  progress_file = file.path(out_dir,
                            paste0("progress_aed_claude", suffix, ".txt")),
  d_pca = d_pca,
  kappa = 1.5,
  run_label = paste0("aed", suffix),
