...
for(treatment in treatment_vec){
  ...
  idx <- which(all_data2$OG_condition %in% c(day_early, treatment))
  keep_vec[idx] <- TRUE
  all_data2$keep <- keep_vec
  all_data2 <- subset(all_data2, keep == TRUE)
  
  tab_mat <- table(all_data2$assigned_lineage, all_data2$OG_condition)
  
  # keep only the relevant cells for this analysis
  keep_vec <- rep(FALSE, ncol(all_data2))
  keep_vec[which(all_data2$OG_condition == day_early)] <- TRUE
  all_data2$keep <- keep_vec
  all_data2 <- subset(all_data2, keep == TRUE)
  ...
  lineage_current_count <- tab_mat[uniq_lineage, day_early]
