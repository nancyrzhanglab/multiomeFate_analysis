...
for(kk in 1:length(time_early_vec)){
  ...
  cell_idx <- which(seurat_obj$Time.point == time_early)
  ...
  for(color_by in c("celltype", "dominant_fate")){
    for(size_by in c("cellsize", "entropy")){
      ...
      # create empty plot
      pdf(file = paste0(plot_folder, "Writeup11b_time-", time_early, "-to-", time_later, "_simplex_color-", color_by_name, "_size-", size_by, ".pdf"),
          width = 10, 
          height = 5, 
          onefile = TRUE)
