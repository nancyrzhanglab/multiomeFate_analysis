...
set.seed(10)
pdf("../../../../out/figures/Writeup6l/Writeup6l_chromvar-DABTRAM_ap1motif-day0size-day10size.pdf", 
...
for(j in 1:ncol(mean_mat)){
  df <- data.frame(day0 = log10(tab_mat[,"day0"] + runif(nrow(tab_mat), min = 1, max = 1.3)),
                   ...
                   chromvar = mean_mat[,j])
  colnames(df)[3] <- colnames(mean_mat)[j]
  p <- GGally::ggpairs(df, 
          lower = list(continuous = GGally::wrap("points", alpha = 0.2, shape = 16))) + ggplot2::theme_bw()
  print(p)
}
dev.off()
...
# make a ton of plots now
set.seed(10)
pdf("../../../../out/figures/Writeup6l/Writeup6l_chromvar-CIS_ap1motif-day0size-day10size.pdf", 
    onefile = T, width = 5, height = 5)
for(j in 1:ncol(mean_mat)){
  df <- data.frame(day0 = log10(tab_mat[,"day0"] + runif(nrow(tab_mat), min = 1, max = 1.3)),
...
set.seed(10)
pdf("../../../../out/figures/Writeup6l/Writeup6l_chromvar-COCL2_ap1motif-day0size-day10size.pdf", 
    onefile = T, width = 5, height = 5)
for(j in 1:ncol(mean_mat)){
  df <- data.frame(day0 = log10(tab_mat[,"day0"] + runif(nrow(tab_mat), min = 1, max = 1.3)),
                   day10_cis = log10(tab_mat[,"day10_CIS"] + runif(nrow(tab_mat), min = 1, max = 1.3)),
                   chromvar1 = mean_mat[,j])
  colnames(df)[3] <- paste0(colnames(mean_mat)[j], "_day0")
  p <- GGally::ggpairs(df, 
...
lineage_idx_list2 <- lapply(lineage_names, function(lineage){
  intersect(which(all_data$assigned_lineage == lineage),
            which(all_data$dataset == "day10_COCL2"))
})

# for each lineage, compute the mean scores
mean_mat2 <- sapply(motif_idx, function(i){
  sapply(lineage_idx_list2, function(idx_vec){
    mean(all_data[["chromvar"]]@data[i,idx_vec])
  })
})
rownames(mean_mat2) <- lineage_names
colnames(mean_mat2) <- name_vec[motif_idx]

options("ggmatrix.progress.bar" = FALSE)

set.seed(10)
pdf("../../../../out/figures/Writeup6l/Writeup6l_chromvar-COCL2_ap1motif-day0size-day10size_expanded.pdf", 
    onefile = T, width = 5, height = 5)
for(j in 1:ncol(mean_mat)){
  df <- data.frame(day0 = log10(tab_mat[,"day0"] + runif(nrow(tab_mat), min = 1, max = 1.3)),
                   day10_cis = log10(tab_mat[,"day10_CIS"] + runif(nrow(tab_mat), min = 1, max = 1.3)),
                   chromvar1 = mean_mat[,j],
                   chromvar2 = mean_mat2[,j])
  colnames(df)[3] <- paste0(colnames(mean_mat)[j], "_day0")
  colnames(df)[4] <- paste0(colnames(mean_mat2)[j], "_day10")
  p <- GGally::ggpairs(df, 
                       lower = list(continuous = GGally::wrap("points", alpha = 0.2, shape = 16)),
                       progress = FALSE)
  p <- p + ggplot2::theme_bw()
