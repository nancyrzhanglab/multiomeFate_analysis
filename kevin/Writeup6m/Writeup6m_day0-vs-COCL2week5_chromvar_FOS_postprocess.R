...
keep_vec[which(all_data$dataset %in% c("day0", "day10_COCL2", "week5_COCL2"))] <- TRUE
...
pdf(paste0("../../../../out/figures/Writeup6m/Writeup6m_COCL2-all_chromvar_", motif_focus, "_violin.pdf"), 
    onefile = T, width = 5, height = 5)

for(var_idx in 1:ncol(chromvar_mat2)){
  df <- data.frame(status = status_vec,
                   score = chromvar_mat2[,var_idx])
  ...
  p1 <- p1 + ggplot2::ggtitle(colnames(chromvar_mat2)[var_idx])
  print(p1)
}
