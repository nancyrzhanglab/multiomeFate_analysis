...
load("../../../../out/kevin/Writeup6m/Writeup6m_differential_peak_COCL2_day0-vs-week5.RData")
...
p1 <- p1 + ggplot2::ggtitle(paste0("COCL2 (Week5 winners vs Day0 losers): Positive peaks")) 
p1 <- p1 + Seurat::NoLegend()

ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6m/Writeup6m_differential_peak_COCL2_day0-vs-week5_positive-peaks.png"),
...
p1 <- p1 + ggrepel::geom_text_repel(data = subset(df, labeling %in% c("1","3")),
                                    ...
                                    max.overlaps = 50)
p1 <- p1 + ggplot2::geom_hline(yintercept = min(log10p_vec[padjust_vec <= 0.05]), linetype = "dashed", color = "red", size = 1.5) 
p1 <- p1 + ggplot2::ggtitle(paste0("COCL2 (Week5 winners vs Day0 losers): Negative peaks")) 
p1 <- p1 + Seurat::NoLegend()

ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6m/Writeup6m_differential_peak_COCL2_day0-vs-week5_negative-peaks.png"),
                p1, device = "png", width = 10, height = 10, units = "in")
