...
load("../../../../out/kevin/Writeup6m/Writeup6m_day0_chromvar2_FOS.RData")
...
stopifnot(nrow(rann_result$nn.idx) == nrow(chromvar_mat))
n <- nrow(rann_result$nn.idx)
i_vec <- unlist(lapply(1:n, function(i){
  rep(i, length = ncol(rann_result$nn.idx))
}))
j_vec <- unlist(lapply(1:n, function(i){
  rann_result$nn.idx[i,]
}))
x_vec <- rep(1/ncol(chromvar_mat), length = ncol(rann_result$nn.idx))
avg_mat <- Matrix::sparseMatrix(i = i_vec, j = j_vec, x = x_vec)

avg_chromvar_mat <- avg_mat %*% chromvar_mat
rownames(avg_chromvar_mat) <- rownames(chromvar_mat)
...
pdf("../../../../out/figures/Writeup6m/Writeup6m_chromvar2-avg_day0_violin_day0cells_FOS.pdf", 
    onefile = T, width = 8, height = 5)

for(j in 1:ncol(avg_chromvar_mat)){
  df <- data.frame(bin = cell_bin_vec,
                   chromvar = avg_chromvar_mat[cell_names,j])
  ...
  p1 <- p1 + ggplot2::ggtitle(paste0("Chromvar score for ", colnames(avg_chromvar_mat)[j]))
  print(p1)
}
...
pdf("../../../../out/figures/Writeup6m/Writeup6m_chromvar2-avg_day0_violin_day0cells_FOS_byday10DABTRAM.pdf", 
    onefile = T, width = 8, height = 5)
