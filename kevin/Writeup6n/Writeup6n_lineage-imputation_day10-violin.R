...
for(treatment in treatment_vec){
  ...
  tab_mat <- tab_mat[order(tab_mat[,paste0("week5_", treatment)], decreasing = T),]
  lineage_idx <- intersect(which(tab_mat[,paste0("day10_", treatment)] >= 4),
                           which(tab_mat[,paste0("week5_", treatment)] >= 20))
  large_lineages <- rownames(tab_mat)[lineage_idx]
  all_data$imputed_count <- imputed_list[[treatment]]
  all_data2 <- subset(all_data, 
                      dataset == paste0("day10_", treatment) & assigned_lineage %in% large_lineages)
  ...
  for(lineage in large_lineages){
    ...
    lineage_name <- paste0(lineage, "_W5:", tab_mat[lineage,paste0("week5_", treatment)])
  ...
  df <- data.frame(lineage = lineage_vec,
                   imputed_count = all_data2$imputed_count)
  ...
  p1 <- p1 + ggplot2::ylab("Log10(Week5 prediction)")
  ...
  ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6n/Writeup6n_", treatment, "-day10_imputation_stepdown-violinplot.png"),
                  p1, device = "png", width = 20, height = 6, units = "in")
