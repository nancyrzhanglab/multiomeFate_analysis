...
g <- igraph::set_vertex_attr(g, name = "lag", index = 5, value = 4)
idx_root <- 4
num_waves <- 10
num_per_wave <- 5
...
res <- generate_data_input(combn_wave_mat, x_sd_biological = 0.5, 
                           x_sd_technical = 0.4, y_sd_technical = 1.5,
                           num_unrelated_x = 100, num_unrelated_y = 50, 
                           time_on = 10, time_windup = 15, 
                           max_lag = 20, min_lag = 10,
                           x_exp_baseline = 0.1, x_unrelated_intervals = 2,
                           x_unrelated_max = 0.1, y_unrelated_max = 1)
...
gene_idx <- 80
peak_idx <- which(df_x$gene == df_y$name[gene_idx])
plot(df_cell$time, rowSums(res$obs_x[,peak_idx]), pch = 16)
plot(df_cell$time, rowSums(res$mean_x[,peak_idx]), pch = 16)
plot(df_cell$time, res$mean_y[,gene_idx], pch = 16)
plot(df_cell$time, res$obs_y[,gene_idx], pch = 16)
