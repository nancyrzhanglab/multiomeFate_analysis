...
result_list <- vector("list", length(gene_vec_all))
...
for(zz in 1:length(gene_vec_all)){
  if(zz %% 100 == 0) {
    save(result_list, date_of_run, session_info,
         file = paste0("../../../../out/kevin/Writeup6i/day0_binomial_extract-", treatment, "_tmp.RData"))
  ...
  # remove peaks with less than 0.1
  if(any(peak_prior <= 0.1) & any(peak_prior >= 0.1)){
    idx <- which(peak_prior <= 0.1)
    peak_mat <- peak_mat[-idx,,drop = F]
  }
  # if the above didn't trigger anything, remove peaks with less than 0.05
