...
spliced_list <- lapply(1:ncol(all_data2), function(j){
  if(j %% floor(ncol(all_data2)/10) == 0) cat('*')
  
  barcode <- colnames(all_data2)[j]
...
spliced_mat <- Matrix::sparseMatrix(i = tmp[,1],
                                    j = tmp[,2],
                                    x = tmp[,3],
                                    dims = c(nrow(spliced_original), ncol(all_data2)))
colnames(spliced_mat) <- colnames(all_data2)
rownames(spliced_mat) <- rownames(spliced_original)
all_data2[["spliced"]] <- Seurat::CreateAssayObject(counts = spliced_mat)
...
unspliced_list <- lapply(1:ncol(all_data2), function(j){
  if(j %% floor(ncol(all_data2)/10) == 0) cat('*')
...
unspliced_mat <- Matrix::sparseMatrix(i = tmp[,1],
                                    j = tmp[,2],
                                    x = tmp[,3],
                                    dims = c(nrow(unspliced_original), ncol(all_data2)))
colnames(unspliced_mat) <- colnames(all_data2)
rownames(unspliced_mat) <- rownames(unspliced_original)
all_data2[["unspliced"]] <- Seurat::CreateAssayObject(counts = unspliced_mat)

Seurat::DefaultAssay(all_data2) <- "Saver"
all_data2 <- Seurat::FindNeighbors(all_data2, 
                                   reduction = "saverpca",
                                   dims = 1:50)

depth <- Matrix::colSums(all_data2[["RNA"]]@counts)
depth <- pmax(depth, 1)
median_depth <- stats::median(depth)
mat <- all_data2[["spliced"]]@counts
mat <- tiltedCCA:::.mult_mat_vec(mat, median_depth/depth)
mat <- mat %*% all_data2@graphs[["Saver_snn"]] 
all_data2[["spliced"]]@data <- mat

mat <- all_data2[["unspliced"]]@counts
mat <- tiltedCCA:::.mult_mat_vec(mat, median_depth/depth)
mat <- mat %*% all_data2@graphs[["Saver_snn"]] 
all_data2[["unspliced"]]@data <- mat
