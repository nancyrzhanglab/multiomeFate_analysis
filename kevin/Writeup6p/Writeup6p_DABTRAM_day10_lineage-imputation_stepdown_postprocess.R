...
peakvi_mat <- all_data[[paste0("peakVI_", treatment)]]@cell.embeddings[cell_names,]
...
png(paste0("../../../../out/figures/kevin/Writeup6p/Writeup6p_",
           treatment, "-", tp_early, 
...
plot(NA, xlim = range(len_vec), ylim = ylim, 
     xlab = "# variables", ylab = "-Loglikelihood", 
     main = paste0("Stepdown variable selection: ", 
                   treatment, "\n(", tp_early, "->", tp_later, ", Stepdown, Testing,\nSelected model of size: ", 
                   length(var_names)-1, ")"))

# mark the selected model
len_range_vec <- range(len_vec[test_idx_vec])
polygon(x = c(len_range_vec, rev(len_range_vec)),
        y = c(1e5, 1e5, -1e5, -1e5),
        col = "lightpink",
        border = NA)
lines(x = rep(len_vec[test_idx_vec[fit_idx]], 2),
      y = c(-1e5, 1e5), 
      lwd = 2, lty = 3)
...
ggplot2::ggsave(filename = paste0("../../../../out/figures/kevin/Writeup6p/Writeup6p_", treatment, "_umap.png"),
...
ggplot2::ggsave(filename = paste0("../../../../out/figures/kevin/Writeup6p/Writeup6p_",
...
p1 <- p1 + ggplot2::ggtitle(paste0(
  ...
  " cells\n(Stepdown from RNA fasttopics, ATAC PeakVI), (Log-scale)",
...
ggplot2::ggsave(filename = paste0("../../../../out/figures/kevin/Writeup6p/Writeup6p_", treatment, "-", tp_early, "_imputation_stepdown_lineage-level_counts_labeled.png"),
...
save(fit, lineage_imputed_count, cell_imputed_count,
     ...
     file = paste0("../../../../out/kevin/Writeup6p/Writeup6p_", treatment, "_", tp_early, "_lineage-imputation_stepdown_concise-postprocessed.RData"))
