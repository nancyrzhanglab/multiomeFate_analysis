...
idx
length(idx)/length(pvalue_vec)

# ########################################
# 
# optim_df <- function(df,
#                      theta,
#                      N,
#                      N0,
#                      z0_vec,
#                      ub){
#   denom <- stats::pchisq(ub, df = df)
#   sample_llvec <- sapply(z0_vec, function(z){
#     stats::dchisq(z, df = df, log = T)
#   })
#   loglik <- N0*log(theta) + (N-N0)*log(1-theta) + sum(sample_llvec) - N0*log(denom)
#   -loglik
# }
# 
# optim_theta <- function(theta,
#                         df,
#                         N,
#                         N0,
#                         z0_vec,
...
# 
# df <- mean(z0_vec)
# theta <- 0.95 * stats::pchisq(ub, df = df)
# 
# max_iter <- 100
# for(iter in 1:max_iter){
#   tmp <- stats::optimize(
#     f = optim_df,
#     interval = c(1,max(z0_vec)),
#     theta = theta,
#     N = N,
#     N0 = N0,
#     z0_vec = z0_vec,
...
# })
# prob_vec <- sapply(2:length(x_vec), function(i){
#   stats::pchisq(x_vec[i], df = df) - stats::pchisq(x_vec[i-1], df = df) 
# })
# plot(hist_res, col = "gray")
# points(midpoints, prob_vec, pch = 16, col = 2)

pvalue_vec <- sapply(lrt_vec_all, function(x){
  1-stats::pchisq(x, df = df)
})
hist(pvalue_vec)
pvalue_adj <- stats::p.adjust(pvalue_vec, method = "BH")
idx <- which(pvalue_adj <= 0.05)
