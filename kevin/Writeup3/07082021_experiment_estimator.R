...
png(file = paste0("../../out/fig/writeup3/07082021_est_rna_prediction.png"),
...
png(file = "../../out/fig/writeup3/07082021_est_time.png",
    height = 1500, width = 1500, res = 300, units = "px")
...
png(file = "../../out/fig/writeup3/07082021_est_atac_umap_timeoverlay.png",
    height = 3000, width = 6000, res = 300, units = "px")
par(mfrow = c(1,2))
set.seed(10)
tmp <- mat_x; tmp <- scale(tmp, center = T, scale = T); pc_res <- stats::prcomp(tmp)
mat_umap <- Seurat::RunUMAP(pc_res$x[,1:50])@cell.embeddings
...
set.seed(10)
tmp <- mat_y; tmp <- scale(tmp, center = T, scale = T); pc_res <- stats::prcomp(tmp)
mat_umap <- Seurat::RunUMAP(pc_res$x[,1:50])@cell.embeddings
plot(mat_umap[,1], mat_umap[,2], asp = T, col = df_cell$branch+1, pch = 16,
     main = "RNA")
for(iter in 1:length(res$list_diagnos)){
...
# now, what if we didn't have a stepsize?
...
pred_y <- .predict_yfromx(mat_x, res$res_g, family = "gaussian")
png(file = paste0("../../out/fig/writeup3/07082021_est_rna_prediction_nostepsize.png"),
    height = 1500, width = 1500, res = 300, units = "px")
image(.rotate(pred_y), zlim = range(mat_y))
graphics.off()
...
png(file = "../../out/fig/writeup3/07082021_est_time_nostepsize.png",
    height = 1500, width = 1500, res = 300, units = "px")
plot(NA, xlim = c(0,ncol(tmp)), ylim = range(tmp))
for(i in 1:ncol(tmp)){
  if(tmp[1,i] <= tmp[2,i]) col = "green" else col = "red"
...
png(file = "../../out/fig/writeup3/07082021_est_atac_umap_timeoverlay_nostepsize.png",
    height = 3000, width = 6000, res = 300, units = "px")
par(mfrow = c(1,2))
set.seed(10)
tmp <- mat_x; tmp <- scale(tmp, center = T, scale = T); pc_res <- stats::prcomp(tmp)
