...
gene_df$true_pval.adj <- stats::p.adjust(gene_df$true_pval, method = "BH")
...
true_gene_vec[which(gene_df$true_pval.adj >= 0.05)] <- FALSE
...
gene_df$naive_pval.adj <- stats::p.adjust(gene_df$naive_pval, method = "BH")
...
est_gene_vec[which(gene_df$naive_pval.adj >= 0.05)] <- FALSE
...
gene_df$true_pval <- .multtest_correction(
  pvalue_vec = tmp,
  null_names = rownames(gene_df)[which(gene_df$true_gene == FALSE)]
)

tmp <- gene_df$naive_pval; names(tmp) <- rownames(gene_df)
gene_df$naive_pval <- .multtest_correction(
  pvalue_vec = tmp,
  null_names = rownames(gene_df)[which(gene_df$naive_gene == FALSE)]
)
