...
plot1 <- plot1 + ggplot2::ggtitle(paste0("DabTram: scVelo UMAP and clustering"))
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup4d/Writeup4d_scvelo_umap_finalclustering-dabtram.png"),
...
plot1 <- Seurat::FeaturePlot(all_data_subset, reduction = "scVelo.umap",
                             features = "pseudotime")
plot1 <- plot1 + ggplot2::ggtitle(paste0("DabTram: scVelo UMAP, Slingshot pseudotime"))
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup4d/Writeup4d_scvelo_umap_slingshot-pseudotime-dabtram.png"),
                plot1, device = "png", width = 6, height = 5, units = "in")

col_palette <- c(rgb(58, 120, 176, alpha = 255*0.5, maxColorValue = 255), 
                 rgb(240, 135, 55, alpha = 255*0.5, maxColorValue = 255), 
                 rgb(83, 157, 64, alpha = 255*0.5, maxColorValue = 255))

curves_list <- slingshot_curve@metadata$curves
col_vec <- rep(NA, ncol(all_data_subset))
col_vec[grep("day0", all_data_subset$dataset)] <- col_palette[1]
col_vec[grep("day10", all_data_subset$dataset)] <- col_palette[2]
col_vec[grep("week5", all_data_subset$dataset)] <- col_palette[3]

png("../../../../out/figures/Writeup4d/Writeup4d_scvelo_umap_slingshot-curves-dabtram.png",
