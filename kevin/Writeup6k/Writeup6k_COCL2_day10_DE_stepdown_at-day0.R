...
cell_losing_idx <- intersect(which(all_data$assigned_lineage %in% names(lineage_score2)[which(lineage_score2 <= -3)]),
                             which(all_data$dataset == "day0"))
...
labeling_vec[names(logpval_vec)[which(logpval_vec >= 3)]] <- "3"
...
p1 <- p1 + ggplot2::ggtitle(paste0("COCL2 Day0 DE based on lineage's mean growth potential at Day10\n(Top 25% to Bottom 25%)")) +
  ggplot2::xlab("Mean difference in SAVER gene exp.: (Best-Worst)") + ggplot2::ylab("T test p-value (-Log10)")
p1 <- p1 + Seurat::NoLegend()

ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6k/Writeup6k_COCL2-day10_stepdown_at-day0_DE_volcano.png"),
