...
cell_winning_idx <- intersect(which(all_data_cocl2$assigned_lineage %in% names(lineage_score)[which(lineage_score > -1.5)]),
                              which(all_data_cocl2$dataset == "day0"))
cell_winning_names <- colnames(all_data_cocl2)[cell_winning_idx]
cell_losing_idx <- intersect(which(all_data_cocl2$assigned_lineage %in% names(lineage_score)[which(lineage_score < -3.5)]),
                             which(all_data_cocl2$dataset == "day0"))
...
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6k/Writeup6k_chromVar_COCL2_day0-win-vs-lose_day10-lineage-imputation.png"),
