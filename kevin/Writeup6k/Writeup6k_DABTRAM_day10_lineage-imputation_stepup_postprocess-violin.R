...
load("../../../../out/kevin/Writeup6k/Writeup6k_DABTRAM_day10_lineage-imputation_stepup_postprocessed.RData")

tab_mat <- table(all_data$assigned_lineage, all_data$dataset)
tab_mat <- tab_mat[order(tab_mat[,"week5_DABTRAM"], decreasing = T),]
lineage_idx <- intersect(which(tab_mat[,"day10_DABTRAM"] >= 4),
                         which(tab_mat[,"week5_DABTRAM"] >= 20))
large_lineages <- rownames(tab_mat)[lineage_idx]
length(large_lineages)
all_data3 <- subset(all_data, 
                    dataset == "day10_DABTRAM" & assigned_lineage %in% large_lineages)
...
for(lineage in large_lineages){
  ...
  lineage_name <- paste0(lineage, "_W5:", tab_mat[lineage,"week5_DABTRAM"])
...
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup6k/Writeup6k_DABTRAM-day10_imputation_stepup-violinplot.png"),
