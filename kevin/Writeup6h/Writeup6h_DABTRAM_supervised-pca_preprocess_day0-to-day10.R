...
treatment <- "DABTRAM"
tier1_lineages <- rownames(tab_mat)[which(tab_mat[,paste0("day10_", treatment)] >= 20)]
...
tier3_lineages <- rownames(tab_mat)[which(tab_mat[,paste0("day10_", treatment)] <= 2)]

tier1_idx <- intersect(
  intersect(which(metadata_mat$assigned_lineage %in% tier1_lineages),
            which(metadata_mat$assigned_posterior >= 0.5)),
  which(metadata_mat$dataset == paste0("day0"))
)
tier2_idx <- intersect(
...
save(spca_res_list, 
     ...
     file = "../../../../out/kevin/Writeup6h/Writeup6h_DABTRAM_supervised-pca_day0-to-day10.RData")
