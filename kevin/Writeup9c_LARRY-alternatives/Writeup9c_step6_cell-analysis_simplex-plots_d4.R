...
for(treatment in treatment_vec){
  load(paste0("~/project/Multiome_fate/out/kevin/Writeup9c/Writeup9c_", treatment, "_from_day", day_early, "_postprocess.RData"))
...
ggtern::ggsave(filename = paste0("~/project/Multiome_fate/out/figures/Writeup9c/Writeup9c_percent-fate_", day_early, "_cell-pairs.png"),
               plot1, 
               device = "png", 
               width = 10, 
               height = 5, 
...
plot1 <- ggtern::ggtern(data = df,
  ...
  ggplot2::labs(x = "Monocyte", 
                ...
                z = "Undifferentiated",
                title = paste0("Percent fate for day ", day_early))
ggtern::ggsave(filename = paste0("~/project/Multiome_fate/out/figures/Writeup9c/Writeup9c_percent-fate_", day_early, "_cell-pairs_by-dominate-fate.png"),
               plot1, 
               device = "png", 
               width = 10, 
               height = 5, 
...
# for each cell, plot by the shannon entropy of each lineage
...
df <- df[which(!is.na(df$dominant_fate)),]
df$dominant_fate <- factor(df$dominant_fate)

df$entropy <- df$entropy + 0.01
...
plot1 <- ggtern::ggtern(data = df,
  ...
  ggplot2::scale_color_manual(values = color_palette) +
  ggtern::theme_showarrows() + 
  ggplot2::scale_size_area(max_size = 1) + 
  ggplot2::labs(x = "Monocyte", 
                y = "Neutrophil",
                z = "Undifferentiated",
                title = paste0("Entropy of observed lineage for day ", day_early))
ggtern::ggsave(filename = paste0("~/project/Multiome_fate/out/figures/Writeup9c/Writeup9c_percent-fate_", day_early, "_cell-pairs_by-entropy.png"),
               plot1, 
               device = "png", 
               width = 10, 
               height = 5, 
