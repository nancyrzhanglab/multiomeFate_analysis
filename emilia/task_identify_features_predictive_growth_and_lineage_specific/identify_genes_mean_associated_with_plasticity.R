...
rownames(exp_mean) <- exp_mean$assigned_lineage
day10_exp_mat <- exp_mean[, seq(2, ncol(exp_mean)-1)]


cor_vec <- sapply(1:ncol(day10_exp_mat), function(j){
  res <- stats::cor.test(exp_mean$normalized_avg_euc_dist_by_shuffle, day10_exp_mat[,j],
...
save(date_of_run, session_info,
     cor_vec,
     file = paste0(dir, "task_identify_features_predictive_growth_and_lineage_specific/outs/", sample, "_gene_exp_mean_day10_plasticity_correlation.RData"))
