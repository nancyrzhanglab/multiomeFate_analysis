...
saver_pca_col_names = paste0('Saver', TREATMENT, 'PC_', 1:NUM_DIM)
for(l in unique(data_use$assigned_lineage)) {
  df = data_use %>% filter(assigned_lineage == l)
  if(nrow(df) >= MIN_CELL) {
    dists = dist(df[, saver_pca_col_names], method = "euclidean")
...
write.csv(d_in_df, paste0(output_dir, SAMPLE_NAME, '/lineage_variability_', SAMPLE_NAME, '.csv'), row.names = F)
write.csv(d_in_shuffled_df, paste0(output_dir, SAMPLE_NAME, '/lineage_variability_shuffled', SAMPLE_NAME, '.csv'), row.names = F)
...
p1 = ggplot(lineage_barcodes, aes(x = n_cells)) + 
  ...
  theme_bw() +
  labs(title = paste0('Lineage size distribution (', SAMPLE_NAME, ')'), y = 'Number of cells')

ggsave(paste0(output_dir, SAMPLE_NAME, '/lineage_size_histogram_', SAMPLE_NAME,'.png'), plot = p1, width = 5, height = 4 )
...
p2 = ggplot(d_in_plot, aes(x = normalized_avg_eud_dist_by_shuffle)) + 
  ...
  theme_bw() +
  labs(title = paste0('Lineage variability (', SAMPLE_NAME, ')'), 
       x = 'Normalized average Euclidean distance')
ggsave(paste0(output_dir, SAMPLE_NAME, '/normalized_avg_eud_dist_', SAMPLE_NAME, '.png'), plot = p2, width = 6, height = 4 )
...
p3 = ggplot(d_in_df, aes(x = n_cells, y = normalized_avg_eud_dist_by_shuffle)) + 
  geom_point() + 
  theme_bw() +
  labs(title = paste0('Lineage variability vs lineage size (', SAMPLE_NAME, ')'), 
