...
data_dir <- '~/Downloads/'
treatment <- 'DABTRAM'
load(paste0(data_dir, 'Writeup6k_', treatment, '_day10_lineage-imputation_stepdown_postprocessed.RData'))
...
all_data <- subset(all_data, dataset == 'day0')
############################
...
gene <- 'CAV1'
saver.data <- all_data[["Saver"]]@scale.data
...
x_vec <- as.data.frame(x_vec)
y_vec <- as.data.frame(y_vec)

colnames(x_vec) <- c('gene')
colnames(y_vec) <- c('gene')
x_vec$cell_id <- rownames(x_vec)
y_vec$cell_id <- rownames(y_vec)
x_vec$category <- 'high'
y_vec$category <- 'low'

comp_df <- rbind(x_vec, y_vec)
ggplot(comp_df, aes(x = category, y = gene)) +
