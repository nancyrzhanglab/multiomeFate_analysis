...
all_data_day10 <- subset(all_data, dataset == 'day10_DABTRAM')
...
lineage_score <- sapply(1:nrow(tab_mat), function(i){
  ...
  cell_names <- colnames(all_data_day10)[which(all_data_day10$assigned_lineage == lineage_name)]
  if(length(cell_names) == 0) return(NA)
  # quantile(all_data_day10$imputed_count[cell_names], probs = 0.75, na.rm = T)
  length(which(all_data_day10$fatepotential_DABTRAM_d10_w5[cell_names] >= 0))
})
...
lineage_score2 <- sapply(1:nrow(tab_mat), function(i){
  ...
  cell_names <- colnames(all_data_day10)[which(all_data_day10$assigned_lineage == lineage_name)]
  if(length(cell_names) == 0) return(NA)
  mean(all_data_day10$fatepotential_DABTRAM_d10_w5[cell_names], na.rm = T)
})
...
lin.winners <- names(lineage_score2)[intersect(which(lineage_score > 0),
                                               which(lineage_score2 >= -1))]
lin.losers <- names(lineage_score2)[lineage_score2 <= -1]

cell_winning_names <- metadat.day0[metadat.day0$assigned_lineage %in% lin.winners, ] %>% rownames()
cell_loser_names <- metadat.day0[metadat.day0$assigned_lineage %in% lin.losers, ] %>% rownames()
...
keep_vec[cell_loser_names] <- "losing_day0"
...
Seurat::DefaultAssay(all_data) <- "chromVar_day0"
all_data_day0 <- subset(all_data, dataset == 'day0')
set.seed(10)
