...
load(paste0(in_dir, 'differential_winner_loser_saver_lineage_specific_adaptation_gene_DABTRAM_t_test_results.RData'))
diff_res_dabtram <- t_test_results

load(paste0(in_dir, 'differential_winner_loser_saver_lineage_specific_adaptation_gene_COCL2_t_test_results.RData'))
diff_res_cocl2 <- t_test_results

load(paste0(in_dir, 'differential_winner_loser_saver_lineage_specific_adaptation_gene_CIS_t_test_results.RData'))
...
ggplot(diff_res_dabtram, aes(x = fold_change, y = neg_log10_p_val)) +
  ...
  ggrepel::geom_text_repel(data = bottom_10_dabtram, aes(label = feature), color = 'blue',nudge_x = -0.2,  nudge_y = 0.5) +
  geom_hline(yintercept = thres_dabtram, linetype = 'dashed', color = 'gray') +
  labs(x = 'log2 FC', y = '-log10 p-value', title = 'day 0 DABTRAM') +
  xlim(-2.2, 2.2) +
  theme_Publication()

ggsave(paste0(in_dir, 'differential_winner_loser_saver_lineage_specific_adaptation_gene_DABTRAM_volcano_plot.png'),dpi = 300, width = 6, height = 6)
...
ggplot(diff_res_cocl2, aes(x = fold_change, y = neg_log10_p_val)) +
  ...
  ggrepel::geom_text_repel(data = bottom_10_cocl2, aes(label = feature), color = 'blue',nudge_x = -0.05,  nudge_y = 0.5) +
  geom_hline(yintercept = thres_cocl2, linetype = 'dashed', color = 'gray') +
  labs(x = 'log2 FC', y = '-log10 p-value', title = 'day 0 COCL2') +
  xlim(-0.5, 0.5) +
  theme_Publication()

ggsave(paste0(in_dir, 'differential_winner_loser_saver_lineage_specific_adaptation_gene_COCL2_volcano_plot.png'),dpi = 300, width = 6, height = 6)
...
ggplot(diff_res_cis, aes(x = fold_change, y = neg_log10_p_val)) +
  ...
  labs(x = 'log2 FC', y = '-log10 p-value', title = 'day 0 CIS') +
  xlim(-0.8, 0.8) +
  theme_Publication()

ggsave(paste0(in_dir, 'differential_winner_loser_saver_lineage_specific_adaptation_gene_CIS_volcano_plot.png'),dpi = 300, width = 6, height = 6)
