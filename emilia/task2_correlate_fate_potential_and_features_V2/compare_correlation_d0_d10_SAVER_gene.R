...
# Wrangle data
# ==============================================================================
dabtram_d10_saver_cor_vec <- dabtram_d10_saver_cor_vec %>% 
  rename(correlation.day10 = correlation,
         p_value.day10 = p.value)

cocl2_d10_saver_cor_vec <- cocl2_d10_saver_cor_vec %>% 
  rename(correlation.day10 = correlation,
         p_value.day10 = p.value)

cis_d10_saver_cor_vec <- cis_d10_saver_cor_vec %>% 
  rename(correlation.day10 = correlation,
         p_value.day10 = p.value)

dabtram_d0_saver_cor_vec <- dabtram_d0_saver_cor_vec %>% 
  rename(correlation.day0 = correlation,
         p_value.day0 = p.value)

cocl2_d0_saver_cor_vec <- cocl2_d0_saver_cor_vec %>% 
  rename(correlation.day0 = correlation,
         p_value.day0 = p.value)

cis_d0_saver_cor_vec <- cis_d0_saver_cor_vec %>% 
  rename(correlation.day0 = correlation,
         p_value.day0 = p.value)

dabtram_comp <- merge(dabtram_d10_saver_cor_vec, dabtram_d0_saver_cor_vec, by = 'row.names')
cocl2_comp <- merge(cocl2_d10_saver_cor_vec, cocl2_d0_saver_cor_vec, by = 'row.names')
cis_comp <- merge(cis_d10_saver_cor_vec, cis_d0_saver_cor_vec, by = 'row.names')
...
p2 <- ggplot(cocl2_comp, aes(x = correlation.day0, y = correlation.day10)) +
  ...
  xlim(-1, 1) +
  ylim(-1, 1) +
  labs(title = 'COCL2 (saver)',
       x = 'Day 0',
       y = 'Day 10') +
  theme_bw()
ggMarginal(p2, margins = "both", size = 10, type = "density",
           fill = c('gray'))
...
p3 <- ggplot(cis_comp, aes(x = correlation.day0, y = correlation.day10)) +
  ...
  xlim(-1, 1) +
  ylim(-1, 1) +
  labs(title = 'CIS (saver)',
       x = 'Day 0',
       y = 'Day 10') +
  theme_bw()
ggMarginal(p3, margins = "both", size = 10, type = "density",
           fill = c('gray'))
