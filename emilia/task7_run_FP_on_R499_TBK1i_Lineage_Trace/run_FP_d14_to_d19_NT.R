...
metadat.NT.dICB <- metadat[metadat$Condition == 'NT dICB D19 InVivo', ]

lineage_size <- metadat.NT.dICB %>% 
...
snapATAC.dimrec.NT.d14 <- snapATAC.dimrec[rownames(metadat.NT.d14), ]

input_df <- metadat.NT.d14 %>% 
  group_by(lineage_barcode_assignment) %>%
...
cell_lineage <- metadat.NT.d14$lineage_barcode_assignment
uniq_lineage <- sort(unique(cell_lineage))

metadat.NT.d14$row.names <- rownames(metadat.NT.d14)
metadat.NT.d14.cur.size <- metadat.NT.d14[, c('row.names', 'lineage_barcode_assignment')]
metadat.NT.d14.cur.size <- merge(metadat.NT.d14.cur.size, input_df, by = 'lineage_barcode_assignment')
rownames(metadat.NT.d14.cur.size) <- metadat.NT.d14.cur.size$row.names
metadat.NT.d14.cur.size <- metadat.NT.d14.cur.size[rownames(metadat.NT.d14), ]

# check if all
all(rownames(snapATAC.dimrec.NT.d14) == rownames(metadat.NT.d14.cur.size))

# prepare the input
snapATAC.dimrec.NT.d14 <- snapATAC.dimrec[rownames(metadat.NT.d14), ]
metadat.NT.d14.cur.size <- metadat.NT.d14.cur.size %>% pull(lineage_current_count)
cell_features <- as.matrix(cbind(snapATAC.dimrec.NT.d14, metadat.NT.d14.cur.size))
# cell_features <- as.matrix(snapATAC.dimrec.NT.d14)
...
colnames(input_df) <- c("day0", 'dICB_NT')
tab_mat <- input_df

fit_res <- multiomeFate:::lineage_cv(
  cell_features = cell_features,
  cell_lineage = cell_lineage,
  future_timepoint = 'dICB_NT',
...
saveRDS(final_fit, '~/Downloads/final_fit_d14_d19_w_d14_size_scaled.rds')
...
saveRDS(cell_imputed_score2, '~/Downloads/final_fit_d14_d19.rds_w_d14_size_scaled.rds')
...
ggplot(comp_df2, aes(x = log10(lineage_future_count.x + 1), y = log10(lineage_current_count + 1)))+
  geom_jitter(width = 0.1) +
  stat_cor() +
  theme_minimal()

write.csv(comp_df2, paste0(out_dir, 'NT_d14_d19_w_d14_size_lineage_size_comparison.csv'), row.names = F)
