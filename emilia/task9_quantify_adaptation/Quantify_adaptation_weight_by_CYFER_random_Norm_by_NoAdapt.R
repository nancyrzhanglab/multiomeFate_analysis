...
fp_gini_cis <- read.csv(paste0(output_dir, 'fate_potential_gini_index_CIS.csv'))
...
ada_index.CIS <- read.csv(paste0(output_dir, 'adaptation_index_CIS_day10_week5_V0.csv'))
...
ada_index.CIS.minmax <- read.csv(paste0(output_dir, 'adaptation_index_CIS_day10_week5_MinMax.csv'))
# 
ada_index.DABTRAM.minmax <- ada_index.DABTRAM.minmax[ada_index.DABTRAM.minmax$dist.min > 0, ]
ada_index.COCL2.minmax <- ada_index.COCL2.minmax[ada_index.COCL2.minmax$dist.min > 0, ]
# =============================================================================
# Wrangle
# =============================================================================
...
ada_index.DABTRAM$adaptation.index.new <- (ada_index.DABTRAM$dist / mean(ada_index.DABTRAM.minmax$dist.min) ) 

fp_gini.COCL2 <- fp_gini[fp_gini$dataset == 'COCL2_d10_w5', ]
ada_index.COCL2 <- merge(ada_index.COCL2, fp_gini.COCL2, by = 'lineage')
ada_index.COCL2$adaptation.index.new <- (ada_index.COCL2$dist / mean(ada_index.COCL2.minmax$dist.min) ) 

ada_index.CIS <- merge(ada_index.CIS, fp_gini_cis, by = 'lineage')
ada_index.CIS$adaptation.index.new <- (ada_index.CIS$dist / mean(ada_index.CIS.minmax$dist.min) )
...
ada_index.plot <- ada_index.plot[, c('lineage', 'adaptation.index.new', 'gini_index', 'dataset', 'n_cells.t2')]
ada_index.CIS <- ada_index.CIS[, c('lineage', 'adaptation.index.new', 'gini_index', 'n_cells.t2')]
ada_index.CIS$dataset <- 'CIS_d10_w5'
ada_index.plot <- rbind(ada_index.plot, ada_index.CIS)
p <- ggplot(ada_index.plot, aes(x = adaptation.index.new, y = gini_index, color = dataset)) +
  geom_point(aes(size = log10(n_cells.t2), fill = dataset), alpha = 0.7) +
  # geom_vline(xintercept = median(ada_index.plot$adaptation.index), linetype = 'dashed', color = 'gray') +
  # geom_hline(yintercept = 0.5, linetype = 'dashed', color = 'gray') +
  stat_cor(aes(group = dataset, color = dataset), label.x = 0.2, label.y.npc = 0.1) +
  xlim(0, max(ada_index.plot$adaptation.index, rm.na=T)) +
  # xlim(0, 1)+
  ylim(0, 1) +
  scale_size_continuous(range = c(1, 10), breaks=c(0, 0.5, 1, 2, 3), guide = 'none') +
  scale_fill_manual(values = c('DABTRAM_d10_w5' = '#9D85BE', 'COCL2_d10_w5' = '#6DC49C', 'CIS_d10_w5' = '#FBD08C'), guide = 'none') +
  scale_color_manual(values = c('DABTRAM_d10_w5' = '#9D85BE', 'COCL2_d10_w5' = '#6DC49C', 'CIS_d10_w5' = '#FBD08C'), guide = 'none') +
  theme_classic() +
  # facet_wrap(~dataset) +
  xlab('Adaptation index') +
  ylab('Fate potential gini index')
...
ggplot(ada_index.plot, aes(x = adaptation.index, y = gini_index, color = dataset)) +
  geom_point(aes(size = log10(n_cells.t2), fill = dataset)) +
  geom_point(data = ada_index.CIS, aes(size = log10(n_cells.t2), fill = 'black')) +
  stat_cor(aes(group = dataset, color = dataset), label.x = 1.2, label.y.npc = 0.1) +
  xlim(min(ada_index.plot$adaptation.index, rm.na=T), max(ada_index.plot$adaptation.index, rm.na=T)) +
  # xlim(0, 1)+
  ylim(0, 1) +
  scale_size_continuous(range = c(1, 10), breaks=c(0, 0.5, 1, 2, 3), guide = 'none') +
...
ggplot(ada_index.CIS, aes(x = adaptation.index, y = gini_index)) +
  geom_point(aes(size = log10(n_cells.t2), fill = dataset)) +
  stat_cor(aes(group = dataset, color = dataset), label.x = 1.2, label.y.npc = 0.1) +
  xlim(min(ada_index.plot$adaptation.index, rm.na=T), max(ada_index.plot$adaptation.index, rm.na=T)) +
  # xlim(0, 1)+
