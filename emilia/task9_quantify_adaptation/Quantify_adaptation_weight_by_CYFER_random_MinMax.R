...
dist_df.min <- data.frame(matrix(nrow = 0, ncol = 4))
colnames(dist_df.min) <- c('lineage', 'dist.min', 'n_cells.t1', 'n_cells.t2')
fp.mid <- median(metadat.time1[[paste0('fatepotential_', treatment, '_', match_name(time1), '_', match_name(time2) )]])
...
for (lin in lin.common) {
  ...
  mean_dist <- lapply(1: 100, function(i) {
    ...
    metadat.time1.low_fp <- metadat.time1[metadat.time1[[paste0('fatepotential_', treatment, '_', match_name(time1), '_', match_name(time2) )]] < fp.mid, ]
    cell.time1.sample <- sample(rownames(metadat.time1.low_fp), n.cell.time1, replace = T)
    cell.time2.sample <- sample(rownames(metadat.time1.low_fp), n.cell.time2, replace = T)
    
    pca.time1.lin.sample <- pca.time1[cell.time1.sample, ]
    pca.time2.lin.sample <- pca.time1[cell.time2.sample, ]
  ...
  dist_df.min <- rbind(dist_df.min, data.frame(lineage = lin,
                                               dist.min = mean_dist.sampleMean,
                                               n_cells.t1 = n.cell.time1,
                                               n_cells.t2 = n.cell.time2))
}

# find maximum distance
dist_df.max <- data.frame(matrix(nrow = 0, ncol = 4))
colnames(dist_df.max) <- c('lineage', 'dist.max', 'n_cells.t1', 'n_cells.t2')
...
for (lin in lin.common) {
  ...
  # Get upper 75% quantile
  mean_dist.sampleUpper <- max(unlist(mean_dist))
  
  dist_df.max <- rbind(dist_df.max, data.frame(lineage = lin,
                                               dist.max = mean_dist.sampleUpper,
                                               n_cells.t1 = n.cell.time1,
                                               n_cells.t2 = n.cell.time2))
}

# Combine all distances
min_max_dist_df <- merge(dist_df.min, dist_df.max, by = c('lineage', 'n_cells.t1', 'n_cells.t2'), all = TRUE)
dist_df <- merge(dist_df, min_max_dist_df, by = c('lineage', 'n_cells.t1', 'n_cells.t2'), all = TRUE)
# dist_df$adaptation.index <- (dist_df$dist - dist_df$dist.min) / (dist_df$dist.max - dist_df$dist.min)


# hist(dist_df$adaptation.index, breaks = 100, 
#      main = paste0('Distance distribution for ', treatment, ' ', time1, ' vs ', time2), 
#      xlab = 'Normalized distance', ylab = 'Frequency')
# 
# quantile(dist_df$adaptation.index)

# Save the distance data frame
write.csv(dist_df, 
          file = paste0(output_dir, 'adaptation_index_', treatment, '_', time1, '_', time2, '_MinMax.csv'), 
          row.names = FALSE)
...
fp_gini <- fp_gini[fp_gini$dataset == paste0(treatment, '_', match_name(time1), '_', match_name(time2)), ]

comp_df <- merge(fp_gini, 
                 dist_df[, c('lineage', 'adaptation.index')], 
                 by = 'lineage')

# comp_df <- comp_df[comp_df$n_cells > 10, ]
ggplot(comp_df, aes(x = adaptation.index, y = gini_index)) +
  geom_point(aes(size = log10(n_cells)), shape = 21) +
