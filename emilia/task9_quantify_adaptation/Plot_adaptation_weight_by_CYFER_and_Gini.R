...
ada_index.DABTRAM$adaptation.index.new <- (ada_index.DABTRAM$dist - min(ada_index.DABTRAM.minmax$dist.min) ) / 
  (max(ada_index.DABTRAM$dist) - min(ada_index.DABTRAM.minmax$dist.min))

fp_gini.COCL2 <- fp_gini[fp_gini$dataset == 'COCL2_d10_w5', ]
ada_index.COCL2 <- merge(ada_index.COCL2, fp_gini.COCL2, by = 'lineage')
ada_index.COCL2$adaptation.index.new <- (ada_index.COCL2$dist - min(ada_index.COCL2.minmax$dist.min) ) / 
  (max(ada_index.COCL2$dist) - min(ada_index.COCL2.minmax$dist.min))
